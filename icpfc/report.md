...
Как обычно, бывает, перед самым началом, люди начали понемногу отваливаться. Отпуска, родственники, лето. В общем, форс мажоры кругом. ¯\\_(ツ)\_/¯.  
Итого нас осталось четверо, из которых двое коммитились только на часть контеста (Я и сам стартовал только со следующего дня). Спасибо Даше, которая внезапно смогла найти пятого человека на два дня.

## 1. Подготовка
К моменту начала, благодаря совместным усилиям, был готов репозиторий, создан базовый проект, в котором даже можно было запускать тесты, установлен канал общения через Slack. В Slack были сразу прилинкованы боты, которые нас нотифицировали про любые новости из Twitter и Reddit, связанные с ICFPC. Весьма удобно и полезно( В прошлом или позапрошлом году мы даже сканировали гитхаб на предмет открытых репозиториев вида icfpc, правда, не пришлодилось :). 
Совсем не хотелось терять время с правами, сборкой и запуском. Должен сказать, что в этом году все прошло более менее гладко. Правда, сильно сказывалось то, что никто в своей работе не использует Kotlin в продакшне. Если кто-то спросит "почему же вы тогда этот язык использовали?", то я отвечу "Потому что ставка была на то, что в команде будут люди, которые с ним работают, но не сложилось. А на переправе, язык не меняют :)".

## 2. Начало
// Условие  
Началось соревнование в 19:00 пятничного вечера, в то время как я тусил на корпоративе. Команда же, к тому времени, относительно быстро прочитала задание и отправилась ужинать :)

23:00(+4Hrs) У нас есть базовые модельки данных, и на коленке написанный решатор, который должен отдать набор команд по заданному условию. Все идут спать. Пятница - рабочий день, и начинать программировать после полноценного рабочего дня очень тяжело (а у кого-то даже был релиз (да-да пятничный релиз)).

## 3. День первый (6 часов после начала)
01:10(+6Hrs) Перед сном Мне приходит в голову "гениальная идея" о том, что нам совсем даже не надо ничего строить. Ведь можно просто развернуть задачу задом наперед, и "разбирать модель". По моему мнению это должно нам сильно упростить логику и убрать необходимость обрабатывать варианты, когда мы себя застраиваем, или получаем пустоты, к которым боты потом не смогут иметь доступа. С этой идеей я засыпаю.

Вставать после корпоратива было очень тяжело :) И погода этому не способствовала. Так что к коллегам по ICFPC я присоединился около 10 часов утра. К этому моменту мы собрались максимальным возможным составом из 5 человек.
...
12:00(+17Hrs) Мы долго обсуждаем вариант с обьеданием и разворотом модели. По каким-то магическим причинам нам кажется, что не может быть все просто. и мы перепроверям возможные корнеркейсы, когда наш алгоритм не сможет работать. К этому времени у нас получается построить карту пространства с высчитанными расстояниями до земли. Модель должна была быть закреплена к земле, и в каждый момент времени каждая точка модели должна иметь путь к "земле". Посчитав минимальное расстояние каждой точки до земли, мы получили информацию о том, в какой последовательности можно безопасно "объедать модель". На какждом шаге - просто объедаем наиболее удаленную от земли часть, так как на этой части ничего другого не держится.

14:00(+19Hrs) Алгоритм вроде бы есть и написан, но проход из точки А в точку Б затруднен тем фактом, что необходимо использовать только заданные варианты ходов, и в который раз мы натыкаемся на то, что "а может. все-таки попробовать выучить и начать использовать [А*](https://en.wikipedia.org/wiki/A*_search_algorithm)?". Бот промахивается, очень долго планирует как ему ходить в заданную точку.

15:30(+20Hrs) Первый работающий алгоритм, у которого есть функция `gotoAndEat`. Для того, чтобы съедать фигуру, мы добавляем дополнительную команду `Erase`, которая в последствии, при развороте набора команд превращается в `Fill`. В процессе мы немного отхватываем с тем, что пропускаем пару команд и не всегда правильно разворачиваем все координаты. In soviet Russia пустая клеточка заполняет бота.

15:50(+20Hrs) Нельзя просто так взять и проитерироваться в тройном вложенном цикле `x in minx..maxx, y in miny..miny, z in minz..maxz`. Слишком много возможностей для простой ошибки в виде копипаста. Уж не помню. зачем нам это надо было, но наш валидатор отказывался пропускать наши решения
```
...
17:21(+22Hrs) Несмотря на то, что мы успешно решаем многие простые задачки, мы замечаем, что наш бот подозрительно долго скитается по простанству для печати. Обнаруживаем, что вместо того, чтобы идти к ближней точке из возможных, он выбирает дальнюю. Несмотря на то, что на визуализации это выглядело очень впечатляюще (работа кипит, бот бегает), данный подход был весьма неэффективен.
```
...
18:02(+23Hrs) Нельзя просто так взять и не забыть тот факт, что мы не можем ходить сквозь заполненные клетки. Из-за того, что мы "объедали" модельку, мы не столкнулись с этой проблемой аж до 18 задачи ¯\\_(ツ)\_/¯.
...
21:00(+26Hrs) В виду того, что Lighting Round закончился, и есть где развернуться. мы распределяем свои силы. Из запланированых задач мы выбрали следующее
...
22:15(+27Hrs) Мы осознаем, что нас бот излишне осторожен, и может строить лишь в 6-ти направлениях из 18 возможных.
...
09:27(+38Hrs) Мы осознаем, что наш алгорим считает все оооочень долго для моделей с количеством точек около миллиона. Простыми вычислениями мы понимаем, что для того, чтобы посчитать все вовремя, нам понадобится машина времени, чтобы запустить решение за два дня до начала соревнований.
...
13:00(+67Hrs) Мы всеми правдами и не правдами пытаемся прикрутить мультиботов к Лешиному алгоритму. Отсутсвие полного верификатора очень сильно бъет по нам, и мы не понимаем, почему наши решения отказываются проходить. 
...
- Jenkins + Command Line сразу. Пусть лучше не пригодится, чем будем на него тратить время
...
- Заставлять подписываться кровью участников, чтобы не искать оных в последний момент :) Все равно не сработает, я знаю.
- Почитать и попробовать язык программирования, на котором будем программировать немного заранее.
- Научиться делать быстрые, проверенные решения )


P.S. Хочу сказать большое спасибо всем участвовавшим, что смогли выделить такой большой промежуток времени для общего блага :). Могли бы сделать больше? Могли %) Но как говорится, человек человеку - волк, а невыспавшийся человек - зомби зомби зомби.
